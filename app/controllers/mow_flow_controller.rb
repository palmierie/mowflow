class MowFlowController < ApplicationController
  ...
  def optimize_list
    ...
    @depot = ScheduledLocation.where(:next_mow_date => @dates).as_json
    ...
    puts "location ids: #{@location_ids}"
    puts "duration_per_location: #{@duration_per_location}"
    puts "number of routes: #{@number_of_routes}"
    ...
    @matrix = ApiCalls.get_matrix(@coordinates_string)
    puts "matrix: #{@matrix}"
    puts "after api call"
  end
